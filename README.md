...
The worker reads only synced `opencode-v1-sqlite` rows, stages up to 2,048 messages at a time, and embeds them in measured 16-example CPU inference batches. Live polling uses the recent-part index installed by `rag migrate`; a complete reconciliation sweep is throttled to every 15 minutes instead of rescanning the full history on every idle poll. Backfill discovery runs as one server-side cursor and prefetches the next page while the current page is embedded. Documents and chunks are staged with bulk inserts and one batched missing-chunk select; a single lookahead page is staged on a separate connection while the current page is embedded. Vector writes use a dedicated writer connection, so the next CPU encode starts while the previous vector batch is being inserted and committed. It persists only a small cursor file, keeps no transcript staging file, and can be run in the foreground with `better-compact rag run --once`. The service requires a Python environment containing `sentence-transformers`, `torch`, and `psycopg[binary]`; use a durable virtualenv rather than `/tmp` for a long-lived installation. Remote PostgreSQL must use certificate-verifying TLS, or the explicit trusted-LAN insecure opt-in already required by sync. See `aidocs/rag_embedding_pilot.md` for the benchmark protocol and storage gate.

Execution backend is controlled independently from the model and storage format. The default `backend: "auto"` selects the Torch model when the configured directory contains Transformer weights, otherwise it uses the ONNX bundle. `compute_dtype: "bfloat16"` is the default when the CPU exposes oneDNN AVX-512 BF16; unsupported hosts fall back to float32 automatically. The measured CPU path uses length bucketing, direct Torch forward calls, and a 16-example inference batch. To force the measured path explicitly:

```sh
better-compact rag setup \
  --backend torch --compute-dtype bfloat16 --length-bucketing \
  --batch-size 16 --message-batch-size 2048 --threads 16 \
  --model-path /absolute/path/to/bge-small-en-v1.5 \
  --python /absolute/path/to/rag-venv/bin/python3
```

The Torch path needs the full Transformer model directory (for example, one containing `model.safetensors`), not an ONNX-only export. Length bucketing groups similarly sized chunks before encoding to reduce padding; it is enabled by default. The backend and dtype are recorded in `audit.pipeline_run`, while the embedding model identity and vector dimension remain unchanged, so switching execution paths does not trigger a destructive re-embedding or mix a new schema into the compaction path. Existing running services keep their current process settings until restarted.
...
The runtime-directory path must be absolute. Its package metadata exposes separate bundled server and TUI entrypoints; the two configuration entries activate the compaction hooks and the in-app selector respectively. No workspace entry, submodule, parent checkout, or prebuilt `dist` directory is required.
